...
- `.migrations/<component>/analysis/component-tree.json` destination.

## Outputs

- Complete `analysis/component-tree.json` with root, ordered nodes, depth, source files, conditional/dynamic child edges, and discovery status.
...
4. Distinguish components from HTML elements, directives, pipes, projected slots, and dynamically rendered components; label each child edge `static`, `conditional`, or `dynamic` with its condition and evidence.
5. Record root-relative depth, de-duplicate shared nodes by stable ID, detect cycles, and preserve ordered child edges.

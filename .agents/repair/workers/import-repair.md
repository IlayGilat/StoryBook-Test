...
The Repair parent invokes this worker only from an explicit compiler/runtime report and bounded module graph. Do not spawn subagents.
...
Receive component name, failure IDs, report path, import chain, expected symbol/standalone contract, original command, affected stage, and write scope.
...
Map each edit to the originating ID and record the broken edge, corrected edge, root cause, and proposed resolution command/reference.
...
Return changed files, ID mapping, command/result, and concerns. The parent owns integration, broader affected-stage validation, reports, state, decisions, and handoff.
